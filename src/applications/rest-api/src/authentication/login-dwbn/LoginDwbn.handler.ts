...
export const LoginDwbnHandlerLive = HttpApiBuilder.handler(
  RestApiSpec,
  "Authentication",
  "loginDwbn",
  ({ payload }) =>
    Effect.gen(function* () {
      ...
      const accessTokenResult = yield* dwbnOAuth2Service.fetchToken(
        payload.code
      );
      ...
      if (Option.isNone(registeredUserOption)) {
        ...
        const newlyRegisteredUser = yield* RegisterUserUseCase({
          payload: {
            ...
            id: idNewUser,
            idDwbn: idTokenPayload.sub,
            idTelegramChat: payload.idTelegramChat,
            isAdmin,
